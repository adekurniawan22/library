        ...
        <div class="col-lg-6">
            <form action="<?= base_url('user/edit/' . $user['user_id']) ?>" method="POST">
                <div class="form-group">
                    ...
                    <select class="form-control <?= ($validation->hasError('role_id')) ? 'is-invalid' : '' ?>" aria-label="Default select example" name="role_id">
                        <option selected disabled>Select role user</option>
                        <?php foreach ($roles as $role) : ?>
                            <option value="<?= $role['role_id'] ?>" <?= set_select('role_id', $role['role_id'], $role['role_id'] == $user['role_id']) ?>><?= $role['role'] ?></option>
                        <?php endforeach ?>
                    </select>
